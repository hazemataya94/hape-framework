...
test-cli: ## Run a new python container, installs hape cli and runs all tests inside it.
	...
	@echo "$$ time docker run -it --rm -e HAPE_MARIADB_HOST='host.docker.internal' --workdir /workspace -v $(shell pwd)/tests:/workspace/tests -v $(shell pwd)/.env:/workspace/.env --entrypoint /bin/bash hape-production -c 'mkdir playground && ./tests/run-test-scripts.sh cli'"
	@time docker run -it --rm -e HAPE_MARIADB_HOST='host.docker.internal' --workdir /workspace -v $(shell pwd)/tests:/workspace/tests -v $(shell pwd)/.env:/workspace/.env --entrypoint /bin/bash hape-production -c 'mkdir playground && ./tests/run-test-scripts.sh cli'
